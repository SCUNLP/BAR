...
class WorkerPlus(Worker):
    ...
    def equip_item(self, target_item):
        ...
        try:
            ...
            # TODO 甚至如果hotbar_id已经在底部栏中，都无需打开背包，直接切换到底部栏即可
            hotbar_id = int(pos_id.split('_')[-1])
            if hotbar_id in range(9):
                ...
                pos_bottom = result  # 可能是空的，也可能这个位置上现在有东西，需要替换

                slot_pos = SLOT_POS_MAPPING[self.current_gui_type]

                # 如果当前位置有东西，找到空背包栏，将要替换的物品放入背包中
                if not has_empty_bar:
                    labels = self.get_labels()
                    for i in range(9):
                        if "resource_" + str(i) in labels:
                            del labels["resource_" + str(i)]
                    inventory_id_none = self.find_in_inventory(labels, 'none')
                    # TODO inventory_id_none也可能找不到，说明整个背包都满了，这时候需要直接交换位置
                    self.pull_item_all(slot_pos, pos_bottom, inventory_id_none)
                ...
                self._call_func('hotbar.{}'.format(hotbar_id + 1))
                # self._attack_continue()  # why need this?
