...
class Worker(object):
    ...
    def pre_open_tabel(self, attack_num=80):
        ...
        while height_2 - height_1 <= 0.419:  # 跳不起来，说明有东西挡住了
            jump_repeat += 1
            if jump_repeat > max_jump_num:
                break
        ...
        # 到这里可能是可以跳起来了，也可能是跳不起来的 --------------------------------------------
        if height_2 - height_1 <= 0.419:  # 跳不起来，说明有东西挡住了
            '''euip pickaxe'''
            # self.obs, _, _, _, self.info = self._step(action)
            # height = self.info['location_stats']['ypos']

            # find pickaxe 但可能是找不到的
            ...
            if inventory_id is not None:
                if inventory_id != 'inventory_0':  # TODO inventory_id == 'inventory_0' 能说明已经装备好了吗？
                    ...
                    if labels['inventory_0']['type'] != 'none':
                        for i in range(9):
                            if "resource_"+str(i) in labels:  # MODIFIED
        ...
        #     self.obs, _, _, _, self.info = self._step(action)
        #     height = self.info['location_stats']['ypos']
        #     if height < 50:  # TODO 为什么非得小于50才去找工具？
        #         # find pickaxe
        #         labels = self.get_labels()
        #         inventory_id_diamond = self.find_in_inventory(labels, 'diamond_pickaxe', 'item')
        #         inventory_id_iron = self.find_in_inventory(labels, 'iron_pickaxe', 'item')
        ...
        #             inventory_id = inventory_id_diamond
        #
        #         # TODO BUG inventory_id是可能找不到的，导致代码崩溃
    ...
    def crafting_shaped(self, target: str, iter_num: int, recipe_info: Dict):
        ...
        for k, v in items.items():
            ...
            if v.get('item'):
                ...
                item_type = 'tag'
            labels = self.get_labels()
            # print(f"find {item_type} - {item} in inventory ...")
